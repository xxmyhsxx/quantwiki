...
slug: quantized-llm-deployment-backends
sources:
  - raw/repositories/serving/2026-08-vllm-project-vllm-568afb3a/source/vllm/model_executor/layers/quantization/__init__.py
  - raw/repositories/serving/2026-07-nvidia-tensorrt-llm-d567f924/source/examples/quantization/README.md
  - raw/repositories/serving/2026-07-mlc-ai-mlc-llm-6474f7db/source/docs/compilation/configure_quantization.rst
  - raw/repositories/serving/2026-07-ggml-org-llama-cpp-b820cc8e/source/tools/quantize/README.md
...
本页依据 vLLM（快照 `568afb3a`）、TensorRT-LLM（快照 `d567f924`）、MLC-LLM（快照 `6474f7db`）、llama.cpp（快照 `b820cc8e`）的官方代码或文档，以及 [[qserve|QServe]] 论文与实现中对运行时要求的描述。所有内容为单一快照的只读核对，本轮没有构建、导出或运行任何框架。
...
[[quantized-matmul-scaling-execution|量化矩阵乘法的缩放与执行路径]] 解释第 4 段的通用原理；[[gguf-block-quantization-formats|GGUF 块量化存储格式]] 是第 2 段的一个完整实例；本页关注第 2、3 段在不同框架之间的差异。
...
**同一算法的不同内核各有一个名字。** `gptq` 与 `gptq_marlin`、`awq` 与 `awq_marlin` 是并列条目。前者对应各自的原生内核，后者走 [[marlin-batched-w4a16-gemm|Marlin]] 的批处理内核。选择哪一个属于性能决策，不改变权重所表示的量化方案；对比加速比时必须写明用的是哪一个。
...
KV cache 通过独立的 `--kv_cache_dtype` 选择 `int8`、`fp8` 或不量化，说明 KV 位宽在部署里是与权重、激活并列的第三个自由度——与 [[mixed-precision-allocation|混合精度分配]] 中「对象不同、预算不同」的判断一致。

框架还提供量化方案的自动搜索：`--autoq_format` 在 `fp8`、`int4_awq`、`w4a8_awq`、`int8_sq` 中挑选，`--auto_quantize_bits` 给出平均权重位宽的约束。文档说明 `int8_sq` 与 `fp8` 不能同时使用，且实际选择由优化问题求解给出。这相当于把 [[mixed-precision-allocation|逐层精度分配]] 的做法内置进框架，只是其候选集合与代价模型由框架固定。

## 5. MLC-LLM：把校准变成独立步骤

MLC-LLM 的量化模式写成短代码（快照 `6474f7db` 的文档）。权重侧格式为 `qAfB(_id)`：`A` 是权重位数、`B` 是激活存储位数、`_id` 区分算法（对称、非对称、AWQ 等）。当前可选 `q0f16`、`q0f32`、`q3f16_1`、`q4f16_1`、`q4f32_1` 与标注为不稳定的 `q4f16_awq`。权重与激活都量化时，CUDA 上提供 `e4m3_e4m3_f16` 与 `e5m2_e5m2_f16`，即两种 FP8（见 [[fp8-and-mx-data-formats|FP8 与 MX 数值格式]]），层输出保持 FP16 后重新量化为 FP8。文档把默认分组量化算法的来源指向 k-bit inference scaling laws 与 LUT-GEMM 两篇工作。
...
注意第 2 步是「运行模型并写回量化参数」，不是离线统计脚本。[[calibration-and-range-selection|校准数据与量化范围选择]] 要求的样本、掩码与用途记录在这里同样是复现的前置条件。

## 6. llama.cpp：格式即部署

llama.cpp 的量化完全体现在 GGUF 文件里：格式名（`Q4_K_M`、`IQ1_M` 等）直接决定块结构与元数据，运行时的后端（CPU、CUDA、Metal、Vulkan）按张量类型分派。量化工具提供逐张量覆盖与重要性矩阵输入，细节见 [[gguf-block-quantization-formats|GGUF 块量化存储格式]]。它的特点是把「选择哪个格式、哪些层降精度」交给使用者在导出时决定，而不是运行时的配置项。

与本页其他框架相比，这条路径的部署资产是文件本身，跨语言与跨设备的可移植性来自格式的标准化程度。

## 7. 加载之外：运行时对量化的额外要求

这一节的约束都来自服务系统的内存与调度结构：缓存被切成固定大小的块、批量按迭代粒度调整、显存不足时需要抢占。这些机制本身的工作原理见 [[serving-memory-and-batching|推理服务的内存管理与批处理]]；理解它们才能判断某个量化方案是「能加载」还是「能在服务里跑出收益」。

**KV cache 的量化参数存放在哪里。** [[qserve|QServe]] 采用与 vLLM、TensorRT-LLM 相同的分页 KV 布局，但因为 KV 位宽更低而使用逐 head 动态量化，于是把每个 head 的 fp16 尺度与零点放在分页中量化 KV 特征之后，支持运行时更新。这说明「KV4」不只是一个位宽，还包含一套页内布局约定；换成静态逐张量量化可以省掉动态估计，但精度条件随之变化。

**同一模型需要不同的配置。** QServe 在 A100 上使用逐通道权重、在 L40S 上使用按组权重，原因是两块 GPU 的 CUDA 核心承担反量化开销的能力不同。部署配置与硬件绑定，不是模型属性。

**推理时不再重算的部分要写清。** 各类校准统计、搜索出的缩放与裁剪阈值在部署时都已固化；而令牌级动态量化（如逐 token 激活尺度、逐 head KV 尺度）仍需运行时统计。把二者混为一谈会导致对开销的错误预期，判断依据见 [[quantized-matmul-scaling-execution|执行路径]]。
