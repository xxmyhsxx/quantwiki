---
title: GGUF 与块量化存储格式
slug: gguf-block-quantization-formats
sources:
  - raw/repositories/serving/2026-07-ggml-org-llama-cpp-b820cc8e/source/gguf-py/gguf/gguf_reader.py
  - raw/repositories/serving/2026-07-ggml-org-llama-cpp-b820cc8e/source/ggml/src/ggml-common.h
  - raw/repositories/serving/2026-07-ggml-org-llama-cpp-b820cc8e/source/ggml/src/ggml-quants.c
  - raw/repositories/serving/2026-07-ggml-org-llama-cpp-b820cc8e/source/ggml/src/ggml.c
  - raw/repositories/serving/2026-07-ggml-org-llama-cpp-b820cc8e/source/tools/quantize/README.md
  - raw/repositories/serving/2026-07-ggml-org-llama-cpp-b820cc8e/source/tools/imatrix/README.md
  - raw/repositories/serving/2026-07-ggml-org-llama-cpp-b820cc8e/source/tools/imatrix/imatrix.cpp
updated: 2026-09-16
---
...
**为什么用两级尺度。** 若子块各自使用独立 fp16 尺度，每 32 个权重需 $4+16=20$ 字节元数据，位宽代价远高于 0.5。把子块尺度再量化到 6 bit、再用超块级 fp16 兜底，就把元数据压到每 256 个权重 16 字节，同时保留子块级自适应。代价是多一次整数乘与偏置修正，以及打包解包的控制流——这正是 [[quantized-matmul-scaling-execution|执行路径]] 里「低比特存储不等于低比特计算」的具体来源。
...
这一步是「格式选择」与「模型结构」的交界：哪些张量重要、哪些层可承受更低精度，是模型相关的工程判断；工具的作用是把判断表达为配置。与逐层混合精度的系统化方法对照见 [[mixed-precision-allocation|混合精度分配]]。
...
即按输入通道累积激活平方（对校准数据求和，使用时按计数取平均）。这与 [[layer-reconstruction-second-order-compensation|层输出重构]] 中 $G=XX^{\mathsf T}$ 的对角元素 $G_{jj}=\sum_t X_{jt}^2$ 是同一个量，只是此处只保留对角、不保留通道间相关。
...
imatrix 的定位是「校准统计改善量化拟合」，与 GPTQ 的二阶补偿有共同的信息来源（激活二阶统计），但机制不同：这里只改变参考量化器的加权最小二乘目标，不做逐列误差补偿，也不更新未量化权重。两者的差别见 [[gptq|GPTQ]] 与 [[layer-reconstruction-second-order-compensation|层重构页]]。
...
- **目的不同。** 这些格式服务于本地推理运行时的存储与内核，设计时同时考虑解码成本与内存带宽；研究论文提出的量化方案常以精度为第一目标，导出时需要映射或重新实现。例如 BiLLM 的 1.08 bit 参数位宽与 IQ1_M 的 1.75 bpw 不是同一口径，前者是参数位宽、后者是包含尺度的实际块尺寸（对照见 [[luq|LUQ]] 的部署讨论与 [[mixed-precision-allocation|混合精度分配]]）。
