...
论文 §4.2 的观察是：Value 矩阵没有显著的离群模式，而 Key 矩阵的每个 head 中存在固定的离群通道，幅值约为其余值的 10 倍。KV8 尚能容忍，KV4 的量化级数不足。做法沿用 [SmoothQuant](smoothquant.md) 的通道缩放，把难度从 Key 转移出去：
...
目标函数按层选择，而不是全局统一，这一点与其后 [FlatQuant](flatquant.md) 用统一的块级目标形成对照。
