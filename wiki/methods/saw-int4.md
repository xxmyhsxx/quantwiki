...
论文明确点名 KIVI 与 Kitty 属于第二类：它们维护固定长度的未量化残差缓冲，形成混合精度 KV cache。在分页池中承载两种精度需要碎片化布局或额外页表，都会破坏既有融合内核的假设。[KIVI](kivi.md) 的做法正是这样——分页友好的替代方案见该页与本节末尾的对照。
...
与 [KIVI](kivi.md) 的对照最有信息量：两者都采用 token 级量化作为可行基础，但 KIVI 用逐通道键量化加未量化残差窗口换取精度，而论文把「混合精度残差缓冲」列为与分页布局冲突的做法，其替代是旋转加统一的 INT4 布局。这不是说 KIVI 无益，而是说两者的设计目标不同——KIVI 追求 2 bit 级别的压缩，SAW-INT4 在 4 bit 上追求零服务开销。阅读两者时应把「压缩到多少位」与「在什么布局下、付出多少系统代价」分开比较。

与 [QuaRot](quarot.md)、[正交旋转与量化](../theory/orthogonal-rotation-and-hadamard-quantization.md) 的关系是同一机制的延续与收缩：旋转抑制离群值的想法相同，但这里把旋转限制在块对角、只作用于键、并融进解码内核；论文也引用 SpinQuant 说明学习旋转的路线，其自身的 Hessian 感知对照显示在该设定下学习旋转的收益不如简单 Hadamard。

与 [QServe](qserve.md) 都强调「低比特方案必须与系统一起设计」，区别在对象与粒度：QServe 的 KV4 是逐 head 动态量化配合分页页内元数据，本方法强调统一 INT4 布局与旋转的无额外算子化。
