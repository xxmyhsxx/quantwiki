...
定义与算例以 Stanford CS336 讲义（本地快照 `lecture_02.py`，含算子算术强度的逐项计算与 roofline 图）为准；临界批量的换算在 MARLIN 论文 §3.1 中给出同形推导（A10、$b_{opt}\approx50$），QServe 论文 §3.1 用同一框架比较 W4A16／W8A8／W4A8。硬件层级结构见 [GPU 的执行模型与存储层次](gpu-execution-and-memory-hierarchy.md)，量化侧的执行分类见 [量化矩阵乘法的缩放与执行路径](../../implementation/quantized-matmul-scaling-execution.md)。
...
**小批量、权重流量主导的解码可以受益于权重压缩。** 单序列的一步线性层近似 GEMV；多序列批处理把 token 行合成 $M>1$ 的 GEMM，权重可跨行复用。只有权重读取主导且没有被反量化等成本抵消时，降低权重位宽才可能近似按比例减少这部分时间。长上下文还可能由 KV 读取主导，不能把权重流量模型套到整个解码阶段。
...
- **必须绑定批量与阶段。** 记录实际线性层的 $M$，即本轮合并处理的 token 行数，而不只记录 prefill/decode 名称。多请求 decode 与分段 prefill 都可能形成 GEMM；[模型计算基础](../model/transformer-autoregressive-inference.md)与 [vLLM 调度](../../implementation/vllm-inference-execution.md)解释阶段和形状的关系。
