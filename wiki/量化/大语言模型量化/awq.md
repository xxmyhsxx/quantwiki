...
sources:
  ...
  - raw/repositories/quantization/2026-06-mit-han-lab-llm-awq-d6e797a4/source/awq/utils/calib_data.py
...
建议阅读顺序：[[linear-layer-input-channel|线性层与输入通道]] → [[uniform-quantization-and-groups|均匀量化与分组]] → 本页；[[diagonal-scaling-equivalent-transform|对角缩放与等价变换]]进一步解释共性原理与计算图条件。

本页讲机制、论文数值与证据边界；**打包约定、内核入口、以及在 vLLM 与 SGLang 里如何加载同一份 AWQ 权重**见 [[awq-implementation|AWQ 的实现核对]]。
...
C0 `quantizer.py:61`。分析式和实现式用途不同，不应把一者无条件替代另一者。均匀量化的一般定义、舍入与裁剪的区别见[[uniform-quantization-and-groups|基础页]]。
...
一般非线性并不满足缩放可交换性；C0 也使用 `ScaledActivation` 在激活后显式相除，并非所有缩放都能无开销融合。Llama 分支的 attention 输出缩放还检查相关投影权重形状是否相等。相关定位：C0 `auto_scale.py:34`、Llama 适配 `auto_scale.py:215`、`apply_scale` 函数 `auto_scale.py:449`。更一般的推导见 [[diagonal-scaling-equivalent-transform|等价变换页]]。
...
因此 P0 所称“无 reconstruction”应结合其不采用 GPTQ 式权重补偿机制理解，不能进一步写成“不比较输出”“不优化任何校准目标”或“不会过拟合”。AWQ 首轮仅局部阅读这两篇依赖；2026-09-14 后续已分别完成全文研读，完整机制与证据见 [[gptq|GPTQ]]、[[smoothquant|SmoothQuant]]。跨方法的对象和比较边界见[[awq-gptq-smoothquant-comparison|三种方法比较]]。

在视觉语言模型中，相同缩放恒等式也可以使用不同的重构评价规则。[[mbq|MBQ]] 用回答/视觉位置的梯度统计加权缩放搜索；其表 4 还区分了图文校准与加权目标的作用。不能把 MBQ 特定实验中的 Pile 基线表现视为 AWQ 无法使用图文校准的固有限制。方法之间真正变化的维度见 [[mbq-vlmq-comparison|重要性加权路线的比较]]。
...
本节的压缩权重、反量化与浮点计算属于 weight-only 执行路径。它与模拟量化、W8A8 整数矩阵乘的区别见 [[quantized-matmul-scaling-execution#3. 三类执行路径|量化的三类执行路径]]；下面保留 TinyChat 特有的实现与测量条件。

TinyChat 是 AWQ 配套的原生内核；同一份 AWQ 权重也可以交给为批处理设计的通用内核执行，例如 [[marlin-batched-w4a16-gemm|Marlin]]，以及 vLLM 中单独列出的 awq_marlin 方法（见 [[quantized-llm-deployment-backends|部署框架与后端支持]]）。两者硬件与批量条件不同，加速比不能互换。
