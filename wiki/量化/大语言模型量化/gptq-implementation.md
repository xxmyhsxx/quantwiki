...
sources:
  ...
  - raw/repositories/quantization/2026-06-ist-daslab-gptq-2d65066e/source/quant_cuda.cpp
  - raw/repositories/serving/2026-08-vllm-project-vllm-568afb3a/source/vllm/model_executor/layers/quantization/utils/gptq_utils.py
  - raw/repositories/serving/2026-08-vllm-project-vllm-568afb3a/source/vllm/model_executor/layers/quantization/auto_gptq.py
  - raw/repositories/serving/2026-06-sgl-project-sglang-2f730e29/source/python/sglang/jit_kernel/gptq_marlin_repack.py
  - raw/repositories/serving/2026-06-sgl-project-sglang-2f730e29/source/python/sglang/srt/layers/quantization/gptq/schemes/gptq_marlin.py
updated: 2026-09-17
---

# GPTQ 的实现核对：量化器、打包与两套服务路径

GPTQ 的论文讲的是「固定一个权重后怎样补偿剩下的」，而工程上要回答的是另外三件事：**校准与量化参数在哪里算出来、权重被打成什么位布局、服务框架拿到这份 checkpoint 后认不认。** 后两件尤其容易出问题——和 [[awq-implementation|AWQ 的实现核对]] 一样，跨引擎能否直接加载取决于打包约定，而不是补偿公式。

算法目标、二阶推导与证据边界见 [[gptq|GPTQ]]；Marlin 使用改造后的 GPTQ 格式并把它变成批处理内核，见 [[marlin-batched-w4a16-gemm|Marlin]]。
...
- 阻尼按**相对量**取：`damp = percdamp * mean(diag(H))`，再 `H[diag,diag] += damp`——即阻尼随权重/Hessian 尺度缩放，不是一个绝对常数（与 [[layer-reconstruction-second-order-compensation|层输出重构]] 里「按平均对角值比例」的记录一致）；
...
repack 的几何（16 行 tile、pack_factor、awq 与 MoE 变体）与 marlin 内核的模板匹配条件见 [[weight-only-dequant-kernels|权重量化反量化内核的契约]]。
