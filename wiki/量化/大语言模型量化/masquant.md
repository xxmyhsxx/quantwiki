...
sources:
  ...
  - raw/repositories/quantization/2026-09-alibaba-efficientai-3d32ae42/source/masquant/README.md
  - raw/repositories/quantization/2026-09-alibaba-efficientai-3d32ae42/source/masquant/main.py
  - raw/repositories/quantization/2026-09-alibaba-efficientai-3d32ae42/source/masquant/quantize/masquant.py
  - raw/repositories/quantization/2026-09-alibaba-efficientai-3d32ae42/source/masquant/quantize/quantizer.py
  - raw/repositories/quantization/2026-09-alibaba-efficientai-3d32ae42/source/masquant/quantize/int_linear.py
  - raw/repositories/quantization/2026-09-alibaba-efficientai-3d32ae42/source/masquant/quantize/svd_utils.py
  - raw/repositories/quantization/2026-09-alibaba-efficientai-3d32ae42/source/masquant/infer_mas.py
  - raw/repositories/quantization/2026-09-alibaba-efficientai-3d32ae42/source/masquant/custom_dataset.py
updated: 2026-09-15
---
...
主要实验量化 Qwen2.5-VL 3B/7B 与 Qwen2.5-Omni 3B/7B 的语言部分；Omni 限于 Thinker，不覆盖 Talker、语音生成器与模态编码器。不能将其称为完整多模态模型所有模块的低比特化，也不是 KV cache 方法。对象边界见 [[vision-language-model-tokens-and-quantization|图文 token 与量化对象]]。（§5.1。）

本页采用论文的右乘权重布局：$X^{(m)}\in\mathbb R^{T_m\times d}$，$W\in\mathbb R^{d\times o}$，$Y^{(m)}=X^{(m)}W$；$m$ 表示 token 所属模态。PyTorch Linear 的存储权重是这里的 $W^\mathsf T$，参见 [[linear-layer-input-channel|线性层与输入通道]]。

[[smoothquant|SmoothQuant]] 使用激活与权重通道范围确定尺度。若混合所有 token 的最大值由某个模态主导，其他模态在不同通道上可能被不均匀地缩小，单个 token 内的有效动态范围反而更差。图 1、4 表明主导模态随层和通道变化，不能简化为“视觉永远比文字大”。该问题涉及通道之间的比例，不是单纯整体幅度大或小。（§4.1。）

对角恒等式本身没有失效：任意可逆对角 $S_m$ 都有

$$X^{(m)}W=(X^{(m)}S_m^{-1})(S_mW).$$

真正的约束是：不同 $S_m$ 通常对应不同 $S_mW$；若都保存，会增加权重副本。只保留一份变换后权重，便不能一般地同时满足所有模态的浮点等价式。参见 [[diagonal-scaling-equivalent-transform|对角缩放的配对条件]]。
...
上式保留线性层机制；实际代码在 Transformer block 输出上计算损失，还存在共享尺度和归一化选项。损失权重、token 数量、每模态均值或总体均值共同决定优化目标，不能只记录 $\lambda_m$。详见 [[calibration-and-range-selection|校准目标与数据]]。
...
完整的数学解释与反例见 [[invertible-transforms-and-kronecker-products|可逆变换与激活加权低秩近似]]。这个结论是**给定校准激活、残差和秩的最优近似**，没有证明任意模态差异天然低秩。可逆左乘保持代数秩；白化能改变谱能量分布与有效秩。图 5 的有效秩下降是特定模型的观察，不是上述定理的必然结果。奇异二阶矩需要伪逆或阻尼，不能继续无条件使用普通逆和原最优性表述。
...
存储也应按位宽核算：每个非文字模态额外保存 $r(d+o)$ 个因子元素，相对 $do$ 个 $b$ bit 主体权重，若因子为 $p$ bit，附加比例约为 $r(d+o)p/(dob)$。教学例：方阵、$r/d=2\%$、主体 4 bit、因子 16 bit 时附加约 16%，而非 4%。尚未计尺度、缓存和其他模块；一般成本见 [[quantized-matmul-scaling-execution|量化矩阵乘法的执行路径]]。
...
与 [[mbq|MBQ]] 的差别是从损失加权走向分别学习表示；与 [[mquant|MQuant]] 的差别是处理变换后权重共享，而分模态量化网格本身未必要求多份权重。[[splitq|SplitQ]] 再把问题拆成通道结构、权重误差和激活误差：它并不是把本方法的模态旁路简单增大。
