...
sources:
  ...
  - raw/repositories/quantization/2026-06-ist-daslab-gptq-2d65066e/source/gptq.py
  - raw/repositories/quantization/2026-06-ist-daslab-gptq-2d65066e/source/quant.py
  - raw/repositories/quantization/2026-06-ist-daslab-gptq-2d65066e/source/opt.py
  - raw/repositories/quantization/2026-06-ist-daslab-gptq-2d65066e/source/datautils.py
  - raw/repositories/quantization/2026-06-ist-daslab-gptq-2d65066e/source/README.md
...
**实现层深入入口：** 量化器参数、权重位打包布局，以及 vLLM／SGLang 的加载与 repack 路径见 [[gptq-implementation|GPTQ 的实现核对]]。

前置阅读：[[linear-layer-input-channel|线性层与输入通道]]、[[uniform-quantization-and-groups|均匀量化与分组]]。[[layer-reconstruction-second-order-compensation|层输出重构与二阶误差补偿]]提供完整的约束优化推导与教学例子。
...
这是单步连续受约束最优，不是整层离散问题的全局最优。只要后续也要舍入、校准分布有偏或数值计算有误，最终质量就仍需检查。逆矩阵的更新及完整拉格朗日推导见[[layer-reconstruction-second-order-compensation|机制页]]。
...
大 batch 下，P 提议先完整反量化再做浮点矩阵乘，举出的 OPT-175B FC2、16×1024 token 案例是反量化开销较小，不是上述低 batch 加速率仍成立。进一步的执行原理见[[quantized-matmul-scaling-execution|量化矩阵乘法的缩放与执行路径]]。

同一量化格式在批量场景下也可以换用别的内核：[[marlin-batched-w4a16-gemm|Marlin]] 使用与原始 GPTQ 略有差别的权重布局，并对 GPTQ 补充了按组裁剪阈值搜索与可变长度校准序列，从而把 W4A16 的接近理论上限加速保持到 batch 约 16–32。格式改造与内核设计是两件事，复现时需要分别记录。
...
研究联系见[[awq-gptq-smoothquant-comparison|AWQ、GPTQ 与 SmoothQuant：对象、机制与证据比较]]。新的实测比较需要统一模型版本、量化配置、校准与评测协议，以及后端和硬件；不同论文的 PPL 与加速倍数不能拼接排名。

## 10. 非对称校准与 token 加权的后续扩展

GPTAQ v3 §4 将受前层量化影响的输入 $U$ 与浮点参考输入 $F$ 区分，重构 $\widehat WU$ 与 $WF$，额外处理输入偏差引入的残差；这与本页同输入的局部重构目标不同。[[vlmq|VLMQ v2]] 默认在该非对称路线中引入 token 因子，形成 $\|(\widehat WU-WF)G\|_F^2$，但部分 INT2 实验又选择 GPTQ 为基础算法。

因此“用到 GPTQ 内核”“基于 GPTQ 补偿”和“使用 GPTAQ 非对称校准”要分开记录。VLMQ 的带权重构与残差推导由其方法页承接；本次对 GPTAQ 仅按需要局部研读，不将其列为已独立完成的全文 ingest。

## 11. 旋转与权重量化的先后关系

[[quarot|QuaRot]] 在固定旋转坐标中重新收集输入并运行 GPTQ；[[spinquant|SpinQuant]] 的主配置则先用 W16 与模拟低比特激活学习旋转，再固定旋转执行 GPTQ。旋转改变待量化权重、激活和相应二阶统计，因此不能直接把原坐标系的量化码或校准矩阵原样当作新坐标的结果。两篇的流程也说明 GPTQ 是可组合的权重补偿阶段，不自行完成激活或缓存的低比特部署。（QuaRot v2 §4 Stage 2a；SpinQuant v4 §4.2、表 3。）
